...
class Database:
    ...
    async def get_notcopy_user(self, user_id):
        ...
        if not user:
            res = {
                "user_id": user_id,
                "last_verified": datetime.datetime(2020, 5, 17, tzinfo=ist_timezone),
                "second_time_verified": datetime.datetime(2019, 5, 17, tzinfo=ist_timezone),
            }
    ...
    async def is_user_verified(self, user_id):
        ...
        pastDate = pastDate.astimezone(ist)
        now = datetime.datetime.now(tz=ist)

        seconds_since_midnight = (now - now.replace(hour=0, minute=0, second=0, microsecond=0)).total_seconds()
        return (now - pastDate).total_seconds() <= seconds_since_midnight
    ...
    async def use_second_shortener(self, user_id, time):
        ...
        if not user.get("second_time_verified"):
            await self.update_notcopy_user(user_id, {
                "second_time_verified": datetime.datetime(2019, 5, 17, tzinfo=ist)
            })
            user = await self.get_notcopy_user(user_id)

        if await self.is_user_verified(user_id):
            pastDate = user["last_verified"].astimezone(ist)
            now = datetime.datetime.now(tz=ist)

            if (now - pastDate) > datetime.timedelta(seconds=time):
                return user["second_time_verified"].astimezone(ist) < pastDate
    ...
    async def use_third_shortener(self, user_id, time):
        ...
        if not user.get("third_time_verified"):
            await self.update_notcopy_user(user_id, {
                "third_time_verified": datetime.datetime(2018, 5, 17, tzinfo=ist)
            })
            user = await self.get_notcopy_user(user_id)

        if await self.user_verified(user_id):
            pastDate = user["second_time_verified"].astimezone(ist)
            now = datetime.datetime.now(tz=ist)

            if (now - pastDate) > datetime.timedelta(seconds=time):
                return user["third_time_verified"].astimezone(ist) < pastDate
    ...
    async def give_free_trial(self, user_id):
        ...
        await self.users.update_one(
            {"id": user_id},
            {"$set": {"id": user_id, "expiry_time": expiry_time, "has_free_trial": True}},
            upsert=True
        )
    ...
    async def remove_group_connection(self, group_id, user_id):
        await self.connection.update_one({'_id': user_id}, {'$pull': {'group_ids': group_id}})
